...
const VideoCard = ({ video }) => {
  ...
  return (
    <div className="mb-4 w-full cursor-pointer">
      ...
      <Link to={`/viewVideo/${video._id}`} className="block">
      ...
      <div className="flex mt-2 gap-2">
        ...
        <Link to={`/viewVideo/${video._id}`} className="flex-1">
          <div className="text-sm">
            <p className="font-semibold line-clamp-2">{video.title}</p>
            <p className="text-gray-600 text-xs">{video.channel.name}</p>
            <p className="text-gray-600 text-xs">
              {video.views} views • {new Date(video.createdAt).toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric' })}
            </p>
          </div>
        </Link>
      </div>
